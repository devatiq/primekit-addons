<?php
/**
 * Render View file for Team Member 3.
 */

 if (!defined('ABSPATH')) exit; // Exit if accessed directly

$primekit_settings = $this->get_settings_for_display();
$primekit_style_three_id = $this->get_id();
...
$primekit_team_phone_number = $primekit_settings['primekit_elementor_teammember_phone_number'];
$primekit_team_email_address = $primekit_settings['primekit_elementor_teammember_email_address'];
$primekit_team_btn_text = $primekit_settings['primekit_elementor_teammember_btn_text'];
$primekit_team_btn_link = $primekit_settings['primekit_elementor_teammember_btn_link'];
$primekit_member_image_dimension = $primekit_settings['primekit_elementor_teammember_image_dimension'];
?>

<!-- Team Member Style 3 Start -->
<div class="primekit-team-member-style-three-area" id="primekit-team-member-<?php echo esc_attr($primekit_style_three_id); ?>">
    <div class="primekit-team-member-style-three-wrap">

        <!-- Team Member Image -->
        <div class="primekit-style-three-team-image">
            <?php
            // Process the image and set default if not available
            $primekit_member_image_url = '';
            ...
            ?>
            <?php if (!empty($primekit_team_btn_link)) : ?>
                        <a href="<?php echo esc_url($primekit_team_btn_link); ?>">
                    <?php endif; ?>
        ...
        <div class="primekit-style-three-team-contents">

            <!-- Team Member Designation -->
            <?php if (!empty($primekit_team_designation)) : ?>
                <h4 class="primekit-style-three-team-designation"><?php echo esc_html($primekit_team_designation); ?></h4>
            <?php endif; ?>

            <!-- Team Member Name -->
            <?php if (!empty($primekit_member_name)) : ?>
                <h3 class="primekit-style-three-team-name">
                    <?php if (!empty($primekit_team_btn_link)) : ?>
                        <a href="<?php echo esc_url($primekit_team_btn_link); ?>">
                    <?php endif; ?>
                    <?php echo esc_html($primekit_member_name); ?>
                    <?php if (!empty($primekit_team_btn_link)) : ?>
                        </a>
                    <?php endif; ?>
                </h3>
